...
# internal imports
from rangers_shop.models import (
    Customer,
    Product,
    ProdOrder,
    Order,
...
def get_order(cust_id):
    ...
    for order in prodorder:
        ...
        prod_data[
            "order_id"
        ] = order.order_id  # want to associate this product with a specific order
        prod_data[
            "id"
        ] = (
            order.prodorder_id
        )  # need to make products unique even if they are the same product
...
def create_order(cust_id):
    ...
    for product in customer_order:
        ...
        current_product = Product.query.filter(
...
def update_order(order_id):
    ...
    prodorder = ProdOrder.query.filter(
        ProdOrder.order_id == order_id, ProdOrder.prod_id == prod_id
    ).first()
